...
function local_aiquestions_get_questions($courseid, $story, $numofquestions, $idiot = 1) {
    ...
    $explanation = "Bitte schreibe $numofquestions Multiple-Choice-Fragen in deutscher Sprache";
    $explanation .= " im GIFT-Format zum folgenden Text, ";
    $explanation .= " GIFT-Format verwendet ein Gleichheitszeichen für die richtige Antwort und ein Tildezeichen für die falsche Antwort am Anfang der Antworten.";
    $explanation .= " Zum Beispiel: '::Frage Titel { =richtige Antwort ~falsche Antwort ~falsche Antwort ~falsche Antwort }' ";
    $explanation .= " Bitte lassen Sie eine leere Zeile zwischen den Fragen. ";

    if ($idiot == 1) {
        $explanation .= " Schreibe die Fragen im richtigen Format! ";
        $explanation .= " Vergiss kein Gleichheits- oder Tildezeichen!";
    }
    ...
    if (isset($result->choices[0]->message->content)) {
        $questions->text = htmlspecialchars($result->choices[0]->message->content, ENT_QUOTES);
...
function local_aiquestions_create_questions($courseid, $gift, $numofquestions, $userid) {
    ...
    foreach ($questions as $question) {
        ...
        $q->timecreated = time();
        $q->timemodified = time();
        $q->questiontext = ['text' => "<p>" . htmlspecialchars($questiontext, ENT_QUOTES) . "</p>"];
